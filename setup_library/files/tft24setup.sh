...
function softwareinstall() {
    echo "Installing Pre-requisite Software...This may take a few minutes!"
                apt-get install -y libts0 1> /dev/null 2>&1 || apt-get install -y tslib 1> /dev/null 2>&1 
    apt-get install -y bc fbi git python3-dev python3-pip python3-smbus python3-spidev evtest libts-bin device-tree-compiler 1> /dev/null  || { warning "Apt failed to install software!" && exit 1; }
    apt-get -y install python3-evdev 1> /dev/null  
}
